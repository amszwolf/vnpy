...
本文件是从 hlm5_all_parallel.py 修改而来，专门用于处理期货数据，支持双向交易。
...
使用示例：
    python hlm5_all_parallel.py  # 使用默认配置（2个品种，1个月数据）
    python hlm5_all_parallel.py --max-tickers 5 --verbose  # 处理5个期货合约
    python hlm5_all_parallel.py --start-date 2025-01-01 --end-date 2025-03-31  # 3个月数据
    python hlm5_all_parallel.py --raw-table tb_futures_rboi_5min  # 切换到5分钟数据
...
def generate_trading_signals(trading_signal_db, ticker, start_date, end_date, enable_bidirectional_trading=True):
    """
    生成交易信号（支持双向交易）
    ...
    print(f"Successfully generate_trading_signals for {ticker}")
