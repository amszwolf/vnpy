...
从 hlm5_scaling_strategy.py 完整移植
支持多种加仓方式的实现和回测
...
5. Aggressive Pyramid (激进金字塔加仓)

⚠️ 从 hlm5_scaling_strategy.py 第39-693行原样移植
"""
...
class ScalingConfig:
    ...
    ⚠️ 从 hlm5_scaling_strategy.py 第39-92行原样复制
    """
...
class PositionTracker:
    ...
    ⚠️ 从 hlm5_scaling_strategy.py 第94-185行原样复制
    """
    
    def __init__(self, ticker: str = 'OI'):
        """
        初始化持仓跟踪器
        ...
        """
        self.ticker = ticker
        self.specs = get_contract_specs(ticker)
...
class ScalingStrategyEngine:
    ...
    ⚠️ 从 hlm5_scaling_strategy.py 第187-412行原样复制
    """
    
    def __init__(self, config: ScalingConfig, ticker: str = 'OI'):
        """
        初始化加仓策略引擎
        ...
        self.cost_calculator = TradingCostCalculator(ticker)
...
def generate_scaling_signals(signals_df: pd.DataFrame,
                            scaling_config: ScalingConfig,
                            ticker: str = 'OI') -> pd.DataFrame:
    """
    生成加仓策略的交易信号
    
    ⚠️ 从 hlm5_scaling_strategy.py 第415-530行简化移植
    
    Parameters:
    -----------
    signals_df : pd.DataFrame
        基础交易信号DataFrame（需包含 datetime, close, Entry_Signal 等列）
    scaling_config : ScalingConfig
        加仓策略配置
    ticker : str
        期货代码
    ...
    if signals_df.empty:
        logger.warning(f"Empty signals dataframe for {ticker}")
        return pd.DataFrame()
    ...
    for i in range(len(df)):
        ...
        timestamp = pd.to_datetime(row['datetime']) if 'datetime' in row else pd.Timestamp.now()
        
        # 获取基础信号
        signal_type = row.get('Entry_Signal', 0)
        
        # 处理信号
        action_result = engine.on_signal(signal_type, current_price, timestamp)
        
        # 记录结果
        df.at[i, 'Scaling_Action'] = action_result.get('action', '')
        df.at[i, 'Scaling_Size'] = action_result.get('size', 0)
        df.at[i, 'Scaling_Position'] = engine.tracker.current_position
        df.at[i, 'Scaling_Level'] = engine.tracker.current_scaling_level
        
        if engine.tracker.current_position != 0:
            df.at[i, 'Avg_Entry_Price'] = engine.tracker.get_average_price()
            df.at[i, 'Unrealized_PnL'] = engine.tracker.calculate_unrealized_pnl(current_price)
        
        if action_result.get('action') in ['exit', 'exit_trailing_stop']:
            df.at[i, 'Realized_PnL'] = action_result.get('pnl', 0.0)
...
def calculate_scaling_performance(signals_df: pd.DataFrame, 
    ...
    ⚠️ 从 hlm5_scaling_strategy.py 第533-639行原样复制
...
# ====================================================================
# 预定义的加仓策略配置
# ⚠️ 从 hlm5_scaling_strategy.py 第642-693行原样复制
# ====================================================================
...
if __name__ == "__main__":
    ...
    for name, config in SCALING_CONFIGS.items():
        print(f"\n{name}:")
        print(f"  {config}")
        print(f"  加仓序列: {config.scaling_sequence}")
        print(f"  加仓条件: {config.add_position_condition}")
        print(f"  加仓阈值: {config.add_position_threshold}")
        print(f"  移动止损: {config.trailing_stop_pct*100:.1f}%")
    
    print("\n" + "=" * 60)
    print("当前使用策略: aggressive_pyramid")
    print("  - 首次开仓: 5手")
    print("  - 第1次加仓: 盈利1%后加3手")
    print("  - 第2次加仓: 盈利1%后加2手")
    print("  - 最大持仓: 10手")
    print("  - 移动止损: 1.5%")
    print("=" * 60)
