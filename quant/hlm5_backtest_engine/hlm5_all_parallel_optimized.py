"""
HLM5 期货数据智能更新系统 (Futures Version with Bidirectional Trading - OPTIMIZED)

本文件是优化版本，增加了开盘和收盘阶段的精细化管理。
...
【新增优化功能】：
7. 开盘阶段优化：
   - 每个交易时段开始（9:00/10:30/13:30/21:00）重新初始化指标计算
   - 开盘后10分钟缓冲期，信号强制为0（避免跳空失真）
   - 只有累积足够K线数量后才允许生成信号
...
使用示例：
    python hlm5_all_parallel_optimized.py  # 使用优化版本
    python hlm5_all_parallel_optimized.py --max-tickers 5 --verbose
    python hlm5_all_parallel_optimized.py --start-date 2025-01-01 --end-date 2025-03-31
    python hlm5_all_parallel_optimized.py --mode full  # 全量更新
...
def generate_trading_signals(trading_signal_db, ticker, start_date, end_date, 
    ...
    close_reason_counter = {'buffer': 0, 'close_time': 0, 'normal': 0}  # 统计信号屏蔽原因
    ...
    for i in range(1, len(df)):
        ...
        if should_close and position != 0:
            ...
            if position > 0:
                ...
                profit_loss = (exit_price - entry_price) / entry_price * 100 * position
            else:
                # 平空仓
                signals_df.loc[i, 'Short_Exit_Signal'] = True
                profit_loss = (entry_price - exit_price) / entry_price * 100 * abs(position)
            
            signals_df.loc[i, 'Exit_Price'] = exit_price
            signals_df.loc[i, 'Profit_Loss'] = profit_loss
            position = 0
            entry_price = 0.0
        ...
        # 2. 检查是否应该生成交易信号（缓冲期/收盘期判断）
        should_signal, signal_reason = session_mgr.should_generate_signal(
            current_timestamp, df, i
        )
        ...
        if should_signal:
            ...
            if '缓冲期' in signal_reason:
                close_reason_counter['buffer'] += 1
    ...
    print(f"Performance Metrics for {ticker}: {performance_metrics}")
    
    # 输出优化统计信息（新增）
    print(f"\n【开盘/收盘优化统计】{ticker}:")
    print(f"  - 缓冲期屏蔽次数: {close_reason_counter['buffer']} 次")
    print(f"  - 收盘强制平仓次数: {close_reason_counter['close_time']} 次")
    print(f"  - 正常交易次数: {close_reason_counter['normal']} 次")
    print(f"  - 总K线数: {len(df)} 根")
    
    print(f"Successfully generate_trading_signals for {ticker} (OPTIMIZED VERSION)")
...
def process_ticker(ticker, db_path, my_raw_db, my_raw_table, start_date, end_date, forecast_days, verbose, config=None):
    """Process a single ticker with smart update strategy"""
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # 使用1分钟数据，处理前2个期货合约（加快测试速度）
        # 数据范围：1个月（2025-01-01 到 2025-01-31）
        main(
            update_mode='smart', 
            enable_realtime=False, 
            max_tickers=2,  # 默认处理2个期货品种
            verbose=True,
            start_date='2025-01-01',
            end_date='2025-01-31',  # 默认1个月数据
            raw_db='tushare',
...
class SmartDataUpdateManager:
    ...
    def _generate_trading_signals(self, ticker: str, start_date: str, end_date: str, enable_bidirectional_trading: bool = True):
        """生成交易信号（支持双向交易）"""
        # 直接调用全局的 generate_trading_signals 函数
        generate_trading_signals(self.db, ticker, start_date, end_date, enable_bidirectional_trading)
