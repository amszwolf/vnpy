"""
HLM5 期货数据智能更新系统 (Futures Version - Close Only Optimization)

本文件是仅收盘优化版本：禁用开盘缓冲期，只保留收盘平仓。
...
使用示例：
    python hlm5_all_parallel_close_only.py  # 只有收盘平仓优化
    python hlm5_all_parallel_close_only.py --max-tickers 5 --verbose
    python hlm5_all_parallel_close_only.py --mode full  # 全量更新
...
def generate_trading_signals(trading_signal_db, ticker, start_date, end_date, 
    ...
    print(f"Successfully generate_trading_signals for {ticker} (CLOSE-ONLY VERSION)")
